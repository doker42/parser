...
class FetchArticles extends Command
{
    ...
    protected const IMPERSONATE_COMMAND = './curl-imper/curl_chrome116';
    ...
    public function __construct(ArticleRepository $articleRepository)
    {
        ...
        $this->cutoffDate = now()->subWeeks(13);
    ...
    function findArticleData($html) : array
    {
        ...
        if ($dateString) {
            ...
            Log::info("Missing date in the article.");
            return [];
        }
        $author = $articleCrawler->filter('article time + a')->first()->text('');
        $content = $articleCrawler->filterXPath('//article/div[2]/div[1]')->first()->html('');

        $tags = $articleCrawler->filter('article a')->each(function (Crawler $node){
            $href = $node->attr('href');
            if ($href) {
                $arr = $this->urlToArray($href);
                $path = $arr['path'];
                if ($path && !$arr['host']) {
                    $pathArr = explode('/', $path);
                    if (count($pathArr) > 1 && in_array('tag', $pathArr)) {
                        return $pathArr[2];
                    }
                }
            };
        });
        // because new is not a tag (its a category)
        $tags = array_merge(['news'], $tags);
    ...
    function processPage($page = 1)
    {
        ...
        if (!$response) {
            ...
            Log::info("Failed to fetch content from the URL.");
            return;
        }
        ...
        if (!$links) {
            ...
            Log::info("No article URLs found.");
            $this->line($response);
            return;
        }
        if ($this->debug) {
...
//                Log::info("We already have this article:  $link");
//                break;
                continue;
            }
            ...
            if (!$articleResponse) {
                ...
                Log::info('Failed to fetch article content from ' . $link);
                break;
            }
            ...
            if (!$article) {
                ...
                Log::info('Failed find article data');
                break;
            }
